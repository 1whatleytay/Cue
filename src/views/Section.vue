<template>
  <div v-if="info" class="text-center">
    <router-link tag="button" :to="`/study/${this.info.name}`" v-bind:class="[ `bg-${info.color}` ]"
      class="w-64 h-16 rounded text-white text-3xl border-none cursor-pointer mx-8">Study</router-link>
      <router-link tag="button" :to="`/builder/${this.info.name}`" v-bind:class="[ `bg-${info.color}` ]"
      class="w-64 h-16 rounded text-white text-3xl border-none cursor-pointer mx-8">New Card</router-link>
